		...
		},
		{
			...
			[
				...
				/*
				{
					name: '이름',
					type: 'STRING',
					description: '스킬명을 적습니다.',
					required: true
				},
				{
					name: '아이콘',
					type: 'STRING',
					description: '아이콘 URL을 적습니다.',
					required: true
				},
				{
					name: '설명',
					type: 'STRING',
					description: '스킬 설명을 적습니다.',
					required: true
				},
				{
					name: '종류',
					type: 'STRING',
					description: '종류를 적습니다.',
					required: true
				},
				{
					name: '시전',
					type: 'STRING',
					description: '시전 시간을 적습니다.',
					required: false
				},
				{
					name: '재사용',
					type: 'STRING',
					description: '재사용 시간을 적습니다.',
					required: false
				},
				{
					name: 'mp',
					type: 'STRING',
					description: 'MP 코스트를 적습니다.',
					required: false
				},
				{
					name: '거리',
					type: 'STRING',
					description: '거리를 적습니다.',
					required: false
				},
				{
					name: '범위',
					type: 'STRING',
					description: '범위를 적습니다.',
					required: false
				},
				{
					name: '레벨',
					type: 'STRING',
					description: '요구 레벨을 적습니다.',
					required: false
				}*/
			]
		}
...
client.on('messageUpdate', async (oldMessage, newMessage) =>
{
	...
	if(newMessage.channel.parent != categorysId.dialog)
	{
					...
					for(var i = 0; i < url.length; i++)
					{
						if(url[i].toLowerCase() == "character")
						{
							...
							if(i + 1 < url.length)
								...
								interaction.editReply({ content: "/인증 [로드스톤 URL]", ephemeral: true });
							break;
						}
					}
					if(!check)
					...
					dataBase.query("SELECT Config FROM UserSaveData WHERE User_Id='" + interaction.member.id +"'", (err, res) =>
					{
						...
						else
						{
							...
							if((userconfig & config.private) == 0)
							{
								...
								interaction.editReply({ content: "자신의 정보를 비공개 하셨습니다.", ephemeral: true });
							}
							else
							{
								userconfig -= config.private;
								interaction.editReply({ content: "자신의 정보를 공개 하셨습니다.", ephemeral: true });
							}
										...
										else
										{
											if((res.rows[0].config & config.private) == 0)
											{
												fetch("https://xivapi.com/character/" + res.rows[0].ffxiv_id + "?columns=Character.FreeCompanyId,Character.FreeCompanyName,Character.ClassJobs,Character.Avatar").then(webData =>
												{
													...
													interaction.editReply({ content: "/플레이어조회 [@맨션]", ephemeral: true });
												});
											}
											else
												interaction.editReply({ content: "해당 플레이어는 비공개로 되어있습니다.", ephemeral: true });
										}
									});
								}
								else
						...
						if(!target.user.bot)
						{
							...
							if(name.length == 2)
							{
								...
								if(server.length == 2)
								{
									...
									interaction.editReply({ content: "/로그 [존] [@맨션]", ephemeral: true });
							}
							else
								interaction.editReply({ content: "/로그 [존] [@맨션]", ephemeral: true });
						}
									...
									webData.json().then(data =>
									{
										if(data.Character.FreeCompanyId != null && data.Character.FreeCompanyId != 0)
										{
											fetch("https://xivapi.com/freecompany/" + data.Character.FreeCompanyId + "?data=FCM").then(fcWebData =>
											{
												...
												interaction.editReply({ content: "/fc [설명]", ephemeral: true });
											});
										}
										else
											interaction.editReply({ content: "FC가 없습니다.", ephemeral: true });
									});
								}).
								catch(error => {
									interaction.editReply({ content: "/fc [설명]", ephemeral: true });
						...
						case 1:
						{
							...
							fetch("https://xivapi.com/linkshell/search?name=" + encodeURIComponent(linkshell) + "&server=" + server[1]).then(webData =>
							{
								webData.json().then(data =>
								{
										...
										if(data.Results[i].Name == linkshell)
										{
											...
											if(linkshellserver == server[1])
											{
												...
												interaction.channel.send({ embeds: [Embed] ,components: [row] }).then(message =>
												{
													...
													.setFooter("메시지 ID : " + message.id);
													client.channels.cache.get(channelsId.log).send({ embeds: [logEmbed] });
													interaction.editReply({ content: "정상적으로 생성되었습니다.", ephemeral: true });
												});
												break;
											}
										}
									...
									if(!check)
									{
										interaction.editReply({ content: "/링크쉘 [타입] [링크쉘 이름]", ephemeral: true });
									}
								});
							});
							break;
					...
					if (interaction.channel.parent.parentId == categorysId.job_battle)
					{
						if (interaction.member.roles.cache.has(interaction.guild.roles.cache.find(r => r.name === "관리자").id))
						{
							if (interaction.channel.name == "PVE 스킬 툴팁")
							{
								...
								/*
								const Embed = new Discord.MessageEmbed()
								.setTitle(interaction.options.get("이름").value)
								.setDescription(interaction.options.get("설명").value.replace(/`/gi,"\n"))
								.setThumbnail(interaction.options.get("아이콘").value);
								const type = interaction.options.get("종류");
								if(type != null)
									Embed.addField("종류", type.value, true)
								else
									Embed.addField('\u200b', '\u200b', true);
								const cast = interaction.options.get("시전");
								if(cast != null)
									Embed.addField("시전 시간", cast.value, true);
								else
									Embed.addField('\u200b', '\u200b', true);
								const recast = interaction.options.get("재사용");
								if(recast != null)
									Embed.addField("재사용 시간", recast.value, true);
								else
									Embed.addField('\u200b', '\u200b', true);
								const range = interaction.options.get("거리");
								if(range != null)
									Embed.addField("거리", range.value, true);
								else
									Embed.addField('\u200b', '\u200b', true);
								const radius = interaction.options.get("범위");
								if(radius != null)
									Embed.addField("범위", radius.value, true);
								else
									Embed.addField('\u200b', '\u200b', true);
								const mp = interaction.options.get("mp");
								if(mp != null)
									Embed.addField("MP", mp.value, true);
								else
									Embed.addField('\u200b', '\u200b', true);
								const level = interaction.options.get("레벨");
								if(level != null)
								...
								switch(interaction.options.get("직업").value)
								{
									case 0:
									{
										...
										for(var i = 0; i<skills.PaladinSkill.length; i++)
										{
											...
											.setThumbnail(skills.PaladinSkill[i].icon);
											if(skills.PaladinSkill[i].type != null)
												Embed.addField("종류", skills.PaladinSkill[i].type, true)
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.PaladinSkill[i].cast != null)
												Embed.addField("시전 시간", skills.PaladinSkill[i].cast, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.PaladinSkill[i].recast != null)
												Embed.addField("재사용 시간", skills.PaladinSkill[i].recast, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.PaladinSkill[i].range != null)
												Embed.addField("거리", skills.PaladinSkill[i].range, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.PaladinSkill[i].radius != null)
												Embed.addField("범위", skills.PaladinSkill[i].radius, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.PaladinSkill[i].mp != null)
												Embed.addField("MP", skills.PaladinSkill[i].mp, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.PaladinSkill[i].level != null)
												Embed.addField("요구 레벨", skills.PaladinSkill[i].level);
											interaction.channel.send({ embeds: [Embed] });
										}
										for(var i = 0; i<skills.TankSkill.length; i++)
										{
											const Embed = new Discord.MessageEmbed()
											.setTitle(skills.TankSkill[i].name)
											.setDescription(skills.TankSkill[i].description.replace(/`/gi,"\n"))
											.setThumbnail(skills.TankSkill[i].icon);
											if(skills.TankSkill[i].type != null)
												Embed.addField("종류", skills.TankSkill[i].type, true)
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.TankSkill[i].cast != null)
												Embed.addField("시전 시간", skills.TankSkill[i].cast, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.TankSkill[i].recast != null)
												Embed.addField("재사용 시간", skills.TankSkill[i].recast, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.TankSkill[i].range != null)
												Embed.addField("거리", skills.TankSkill[i].range, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.TankSkill[i].radius != null)
												Embed.addField("범위", skills.TankSkill[i].radius, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.TankSkill[i].mp != null)
												Embed.addField("MP", skills.TankSkill[i].mp, true);
											else
												Embed.addField('\u200b', '\u200b', true);
											if(skills.TankSkill[i].level != null)
												Embed.addField("요구 레벨", skills.TankSkill[i].level);
											interaction.channel.send({ embeds: [Embed] });
										}
										for(var i = 0; i<skills.PaladinTrait.length; i++)
										{
											const Embed = new Discord.MessageEmbed()
											.setTitle(skills.PaladinTrait[i].name)
											.setDescription(skills.PaladinTrait[i].description.replace(/`/gi,"\n"))
											.setThumbnail(skills.PaladinTrait[i].icon)
											.addField("요구 레벨", skills.PaladinTrait[i].level);
											interaction.channel.send({ embeds: [Embed] });
										}
										break;
									}
...
client.on("messageCreate", async (msg) =>
{
	...
	switch(cmd[0])
	{
		...
		case "거래":
		{
			if (msg.channel == channelsId.trade)
			{
				if(cmd.length == 2)
				{
					const itemName = msg.content.slice(prefix.length + cmd[0].length + 1);
					const name = msg.member.displayName.split(" ", 2);
					const server = name[1].split("@", 2);
					var DataCenterName;
					for(var j = 0; j < dataCenterNames.length; j++)
					{
						const role = msg.guild.roles.cache.find(r => r.name === dataCenterNames[j].kor);
						if(msg.member.roles.cache.has(role.id))
						{
							DataCenterName = dataCenterNames[j].kor;
							break;
					...
					const Embed = new Discord.MessageEmbed()
					.setColor(getDataCenterColor(msg.member, msg.guild))
					.setTitle(itemName)
					.setAuthor(msg.member.displayName, msg.author.displayAvatarURL())
					.addField("판매자", "<@" + msg.member.id + ">", true)
					.addField("데이터 센터", DataCenterName, true)
					.addField("월드", server[1], true)
					.setTimestamp();
					const Button1 = new Discord.MessageButton()
					...
					msg.channel.send({ embeds: [Embed] ,components: [row] }).then(message =>
					{
						...
						.setTitle(msg.channel.name)
						.setAuthor(msg.author.tag, msg.author.displayAvatarURL())
						.setDescription("<@" + msg.member.id + ">님이 [" + itemName + "]("+ message.url +") 거래글을 게시하셨습니다.")
						.setTimestamp()
						.setFooter("메시지 ID : " + message.id);
						client.channels.cache.get(channelsId.log).send({ embeds: [logEmbed] });
					});
					...
					msg.reply("!거래 [제목]").then(message => { setTimeout(() => message.delete(), 10000); });
			}
			break;
		}
		case "거래설명":
		...
		else
		{
			...
			if(data.Character.Bio == msg.member.id)
			{
				if(msg.member.nickname != data.Character.Name + "@" + data.Character.Server)
				{
					...
					setRole(msg, msg.member, msg.guild.roles.cache.find(r => r.name === "⁣           데이터 센터           ⁣"));
			...
			{
				msg.editReply({ content: "당신의 DM으로 인증코드가 전송되었습니다.", ephemeral: true });
